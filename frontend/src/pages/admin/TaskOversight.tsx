...
const TaskOversight: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-ink-text">Platform Task Oversight</h1>
        <p className="text-xs text-ink-muted">Monitor all active & closed tasks across businesses on TaskHub.</p>
      </div>
        ...
        <table className="w-full text-left text-xs text-ink-muted">
          ...
          <tbody className="divide-y divide-hairline">
            {isLoading ? (
              ...
              tasks.map((t) => (
                <tr key={t.id} className="hover:bg-paper-bg transition-colors">
                  <td className="p-4">
                    <span className="font-bold text-ink-text block">{t.title}</span>
                    <span className="text-[10px] font-mono text-ink-muted">{t.id}</span>
                  </td>
                  <td className="p-4 text-ink-muted">{t.businessCompany || t.businessName || 'Business'}</td>
                  <td className="p-4 text-ink-muted">{t.category}</td>
                  <td className="p-4">
                    <StatusBadge status={t.status} size="sm" />
                  </td>
                  <td className="p-4 text-right font-bold text-moss-deep">{formatCurrency(t.reward)}</td>
                </tr>
              ))
            )}
          </tbody>
