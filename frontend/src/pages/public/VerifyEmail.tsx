  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const role = (window as any).__pendingRole || params.get('role') || 'WORKER';
      ...
      <div className="max-w-md w-full p-8 rounded-3xl bg-paper-bg border border-hairline space-y-6 shadow-xl relative overflow-hidden">
        ...
        <div className="text-center space-y-2 relative z-10">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-moss-deep via-moss-primary to-moss-primary p-0.5 mx-auto shadow-lg shadow-moss-primary/15">
            <div className="w-full h-full bg-paper-bg rounded-[14px] flex items-center justify-center">
              <ShieldCheck className="w-7 h-7 text-moss-deep" />
            </div>
          </div>
          <h2 className="text-2xl font-extrabold text-ink-text tracking-tight">Verify Your Email</h2>
        ...
        <div className="space-y-4 relative z-10">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full !pl-11 py-3 text-xs rounded-xl bg-paper-bg border border-moss-sage text-ink-text placeholder:text-ink-muted/70 focus:ring-2 focus:ring-moss-primary/30 focus:border-moss-primary outline-none transition-all"
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-ink-text mb-1.5">6-Digit Verification Code</label>
            <div className="relative">
              <KeyRound className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-muted z-10 pointer-events-none" />
              <input
                type="text"
                inputMode="numeric"
                maxLength={6}
                ...
                placeholder="000000"
                className="w-full !pl-11 text-center tracking-[0.5em] py-3 text-base font-bold rounded-xl bg-paper-bg border border-moss-sage text-ink-text placeholder:text-ink-muted focus:ring-2 focus:ring-moss-primary/30 focus:border-moss-primary outline-none transition-all"
              />
            </div>
          </div>

          {devMode && (
            <div className="p-3 rounded-xl bg-moss-sage/30 border border-moss-sage text-[11px] text-moss-deep flex items-start gap-2">
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-gradient-to-r from-moss-deep to-moss-primary text-white font-bold text-xs shadow-lg shadow-moss-primary/25 flex items-center justify-center gap-2 hover:opacity-95 transition-all disabled:opacity-60"
          >
            {isVerifying ? (
              <>
          ...
          <button
            ...
            disabled={isSending}
            className="w-full py-2.5 rounded-xl bg-paper-bg border border-hairline text-ink-text text-xs font-semibold flex items-center justify-center gap-2 hover:bg-moss-light/40 transition-all disabled:opacity-60"
          >
            {isSending ? (
          ...
          <div className="flex justify-center gap-4 text-[11px] pt-1">
            <Link to="/register" className="text-moss-deep hover:underline font-semibold flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> Wrong email?
            </Link>
            <Link to="/login" className="text-ink-muted hover:text-ink-text font-medium">
              Back to sign in
            </Link>
          </div>
        </div>
